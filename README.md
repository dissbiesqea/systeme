...
### Hеобходимо запустить контейнер 
### Из под контейнера запускаем:
```bash
composer install
```
...
### подгружаются фикстуры
```bash 
php bin/console doctrine:fixtures:load --purge-with-truncate
```
Данных много добавлять не стал думаю для наглядности их будет достаточно

Тесты сделать не успел, если нужно смогу сделать в понедельник

Также добавил документацию можно увидеть по ссылке http://127.0.0.1:8337/api/doc
